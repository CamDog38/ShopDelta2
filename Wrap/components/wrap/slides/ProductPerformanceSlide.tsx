...
export function ProductPerformanceSlide({ slide }: { slide: Slide }) {
  ...
  return (
    <div className="relative flex min-h-full w-full flex-col justify-start px-4 sm:px-10 py-4 sm:py-6 pb-8">
      <motion.div
      ...
      <div className="relative z-10 flex flex-col gap-3 sm:gap-4">
        ...
        >
          ...
          {slide.subtitle && (
            <p className="mt-0.5 sm:mt-1 text-[10px] sm:text-xs text-slate-100/85">{slide.subtitle}</p>
          )}
        </motion.div>

        <motion.div
          className="grid grid-cols-3 sm:grid-cols-4 grid-rows-4 sm:grid-rows-3 gap-1 rounded-xl overflow-hidden"
        ...
        <motion.div
          className="flex justify-center gap-4 sm:gap-6 text-[10px] sm:text-xs mt-2"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          transition={{ delay: 0.8 }}
        >
          <div className="flex items-center gap-1.5 sm:gap-2">
            <div className="w-2.5 h-2.5 sm:w-3 sm:h-3 rounded bg-emerald-500/60" />
            <span className="text-slate-400">Growth</span>
          </div>
          <div className="flex items-center gap-1.5 sm:gap-2">
            <div className="w-2.5 h-2.5 sm:w-3 sm:h-3 rounded bg-red-500/60" />
